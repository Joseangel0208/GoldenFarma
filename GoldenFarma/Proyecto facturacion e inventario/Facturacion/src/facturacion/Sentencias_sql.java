...
public class Sentencias_sql {
    ...
    public boolean insertar(String datos[], String insert){
      boolean estado = false;
       try {
            ps = con.obtenerConexion().prepareStatement(insert);
     ...
     public Object [][] GetTabla(String colName[], String tabla, String sql){
      ...
      try{
         ps = con.obtenerConexion().prepareStatement("select count(*) as total from " + tabla);
         res = ps.executeQuery();
         res.next();
    ...
    return data;
     ...
     int registros = 0;
     try{
         ps = con.obtenerConexion().prepareStatement("SELECT count("+campo+") as total  " + from_where);
         res = ps.executeQuery();
         res.next();
         registros = res.getInt("total");
         res.close();
    ...
    String datos ="";
      try{
         ps = con.obtenerConexion().prepareStatement(sentenciasql);
         res = ps.executeQuery();
         while(res.next()){
